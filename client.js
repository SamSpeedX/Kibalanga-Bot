...
const connect = async () => {
   ...
   global.client = Socket({
      ...
      browser: ['KIBALANGA', 'safari', '1.0.0'],
      auth: state,
   ...
   client.ev.on('group-participants.update', async (room) => {
      ...
      if (room.action == 'add') {
         if (groupSet.localonly) {
            if (global.db.users.some(v => v.jid == member) && !global.db.users.find(v => v.jid == member).whitelist && !member.startsWith('255') || !member.startsWith('255')) {
               client.reply(room.id, Func.texted('bold', `Sorry @${member.split`@`[0]}, this group is only for TANZANIA people and you will removed automatically.`))
